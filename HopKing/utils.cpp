...
int utils::GenerateRandomRange(int min, int max)
{
	return (rand() % ((max + 1 - min)) + min);
}
float utils::GenerateRandomRange(float min, float max)
{
	return float((rand() % (int((max - min) * 10) + 1) + int(min * 10)) / 10.0f);
}

float utils::GenerateNegativeRandomRange(float min, float max)
{
	return float(int(min) + rand() % int(max - min + 1));
}
