...
void Texture::DrawTextureWithParalax(const Point2f& texturePos, const float parallaxAngle, const Point2f& referencePoint) const
{
	const Vector2f deltaPos{ texturePos.x - referencePoint.x, texturePos.y - referencePoint.y };

	const Point2f parallaxPos{ texturePos.x + (deltaPos.x * parallaxAngle),
		texturePos.y + (deltaPos.y * parallaxAngle) };
	this->Draw(parallaxPos);
}
